<script id="js-feed-item-template" type="text/x-handlebars-template">
    ...
    fluent
    {{#if pinned}}feed-list-item--state-pinned{{/if}}
    {{#if feedIcon }}hasIcon{{/if}}
"
    ...
    <div data-feed-item-id="{{ id }}" class="js-tag-feed-item feed-item-action-icons
        {{#if item.tag }} style="visibility: visible; color:#{{item.tag.color}}" title="{{item.tag.name}}" {{/if}}">
    ...
    {{#if feedIcon }}
    <div class="feed-icon" style="background-color:{{ color }}">
